...
- **Invitation accept-link page** (`/accept-invitation/...`) is an informational placeholder. Admins *can* invite users (it creates the account + emails a link via the configured Mailtrap sandbox), but completing onboarding through the public link needs a backend accept endpoint that's a V1 follow-up. For testing, use the seeded **owner** account, or have an admin create accounts directly.
