...
8. **Team access + cross-team approval (team-access P1–P10, 2026-08-11)**: the workspace is **team-scoped** — a Member/Guest sees ONLY the team(s) they belong to on **Settings → Teams** (admins see everything). Only assignees, the creator, the owning team's **Head**, or an admin may **edit** a task (everyone else gets a 🔒 View-only drawer); every change shows in the task's **Activity**. Assigning someone from ANOTHER team creates an **approval request** (Inbox → **Requests** tab, or the drawer's **Assignment approval** panel: accept / decline / query / answer, 7-day expiry, email+push at each step). Operator guide (Bangla): `TEAM_GUIDE.md`. ⚠️ Fresh installs via plain `db:seed` stay OPEN (everyone sees everything) until you apply `database/upgrades/019+020`; **`db:seed:demo` seeds the TIGHTENED model directly** — 6 teams with heads + rosters and one live cross-team request (nusrat → jhankar, query pending) so every approval surface has data on first login.
