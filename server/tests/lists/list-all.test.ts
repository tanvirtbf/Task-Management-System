...
describe("GET /api/v1/lists", () => {
    ...
    describe("Collection pagination", () => {
        it("caps a large set at the default limit and pages through it", async () => {
            ...
            let pages = 1;
            while (cursor) {
                const next = await client.get(url(`cursor=${encodeURIComponent(cursor)}`));
