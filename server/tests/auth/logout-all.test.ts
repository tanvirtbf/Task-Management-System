 ...
 *
 * Patterns mirror `tests/auth/logout.test.ts`:
 ...
 *
 * Each test exercises real bcrypt (login → logout-all chain) and the first
...
const POST_LOGOUT_ALL = "/api/v1/auth/logout-all";
...
describe("POST /api/v1/auth/logout-all", () => {
    ...
    describe("Happy path", () => {
        ...
        it("revokes every active session for the user", async () => {
            const u = await makeUser();
            const http = await oneOff();
            // Five logins → five active sessions across "devices".
            const tokens: LoginTokens[] = [];
            for (let i = 0; i < 5; i++) {
                tokens.push(await loginAndGetTokens(http, u.email, u.password));
            }
            expect(await countActiveSessions(u.id)).toBe(5);
            ...
            // The 5 rows are still present, just revoked.
            expect(await countSessions(u.id)).toBe(5);
            for (const t of tokens) {
                const row = await findSessionById(t.sessionId);
                expect(row!.revokedAt).not.toBeNull();
            }
        });

        it("leaves other users' sessions untouched", async () => {
            const u = await makeUser({ email: "self@x.test" });
            const other = await makeUser({ email: "other@x.test" });
            const http = await oneOff();
            const me = await loginAndGetTokens(http, u.email, u.password);
            const them = await loginAndGetTokens(
                http,
                other.email,
                other.password,
            );
            ...
            const otherRow = await findSessionById(them.sessionId);
            expect(otherRow!.revokedAt).toBeNull();
            expect(await countActiveSessions(other.id)).toBe(1);
        });
    ...
    describe("Authentication", () => {
        it("returns 401 auth.missing_token when Authorization is absent", async () => {
            const http = await oneOff();
            const res = await http.post(POST_LOGOUT_ALL);
    ...
    describe("Session lifecycle", () => {
        it("returns 204 when the user has zero active sessions left", async () => {
            // Login, then mark every session row revoked directly in DB.
            const u = await makeUser();
            const http = await oneOff();
            const { accessToken } = await loginAndGetTokens(
                http,
        ...
        it("revokes all five active sessions when the user has many devices", async () => {
            const u = await makeUser();
            const http = await oneOff();
            const tokens: LoginTokens[] = [];
            for (let i = 0; i < 5; i++) {
        ...
        it("ignores the calling session being pre-revoked and still revokes the rest", async () => {
            const u = await makeUser();
            const http = await oneOff();
            const me = await loginAndGetTokens(http, u.email, u.password);
            const other = await loginAndGetTokens(http, u.email, u.password);

            // Manually revoke the calling session BEFORE the request.
            const db = getDb();
            await db
                .update(sessions)
                .set({ revokedAt: new Date() })
            ...
            expect(res.status).toBe(204);
            // Other session — initially active — should now also be revoked.
            const otherRow = await findSessionById(other.sessionId);
    ...
    describe("Tenant isolation", () => {
        it("user A logging out everywhere does not affect user B in another workspace", async () => {
            ...
            expect(await countActiveSessions(userB.id)).toBe(1);
        });
    });

    // ─── i. Concurrency ───────────────────────────────────────────────────────
    describe("Concurrency", () => {
        it("two sequential logout-all calls with the same Bearer both return 204", async () => {
            const u = await makeUser();
            const http = await oneOff();
            const { accessToken } = await loginAndGetTokens(
                http,
        ...
        it("10 parallel logout-all calls with the same Bearer all return 204 and converge to 0 active sessions", async () => {
            const u = await makeUser();
            const http = await oneOff();
            const { accessToken } = await loginAndGetTokens(
                http,
        ...
        it("logout-all then login again inserts a fresh session row, preserving the revoked ones", async () => {
            const u = await makeUser();
            const http = await oneOff();
            ...
            const { sessionId: newSid } = await loginAndGetTokens(
                http,
                u.email,
                u.password,
            );
            expect(newSid).not.toBe(a.sessionId);
            expect(newSid).not.toBe(b.sessionId);

            // 3 rows total: 2 revoked + 1 fresh active.
            expect(await countSessions(u.id)).toBe(3);
    ...
    describe("Side effects", () => {
        ...
        it("does NOT update users.last_login_at on logout-all", async () => {
            const u = await makeUser();
            const http = await oneOff();
            const { accessToken } = await loginAndGetTokens(
                http,
    ...
    describe("Request-Id", () => {
        ...
        it("echoes a client-supplied X-Request-Id verbatim on the 204 success", async () => {
            ...
            const rid = "rid-logout-all-test-12345";
            const res = await http
                .post(POST_LOGOUT_ALL)
    ...
    describe("Exploratory", () => {
        it("returns 404 with the spec envelope for GET on the logout-all URL", async () => {
            const http = await oneOff();
            const res = await http.get(POST_LOGOUT_ALL);

            expect(res.status).toBe(404);
            expect(res.body.error.code).toBe("route.not_found");
        });

        it("returns 404 for PUT/DELETE/PATCH on the logout-all URL", async () => {
            const http = await oneOff();
            for (const verb of ["put", "delete", "patch"] as const) {
                const res = await http[verb](POST_LOGOUT_ALL);
                expect(res.status).toBe(404);
                expect(res.body.error.code).toBe("route.not_found");
            }
        });

        it("rejects bad Authorization scheme deterministically (Basic, not Bearer)", async () => {
            const u = await makeUser();
            const http = await oneOff();
            const { accessToken } = await loginAndGetTokens(
                http,
        ...
        it("survives 50 parallel logout-all calls from 50 distinct users", async () => {
            const http = await oneOff();
            const tokens: { accessToken: string; userId: string }[] = [];
            for (let i = 0; i < 50; i++) {
                ...
                tokens.push({ accessToken, userId: u.id });
            }
            ...
            // No cross-user interference: every user is now at 0 active sessions.
            for (const t of tokens) {
                expect(await countActiveSessions(t.userId)).toBe(0);
            }
        });
        ...
        it("after logout-all, /refresh with a now-revoked cookie returns 401 (and any remaining active sessions get reuse-mass-revoked)", async () => {
            const u = await makeUser();
            const http = await oneOff();
            const a = await loginAndGetTokens(http, u.email, u.password);
            ...
            // Refresh with the (now revoked) cookie.
            ...
            // Already fully revoked → the mass-revoke is a no-op; count stays 0.
            expect(await countActiveSessions(u.id)).toBe(0);
        });

        it("accepts a JWT that carries extra harmless claims", async () => {
            const u = await makeUser();
