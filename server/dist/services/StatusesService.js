...
class StatusesService {
    ...
    statuses;
    logger;
    constructor(db, lists, statuses, logger) {
        this.db = db;
        this.lists = lists;
    ...
    async createForList(input) {
        ...
        try {
            ...
            if (isDuplicateKeyError(err)) {
                throw errors_1.AppError.conflict("status.duplicate", `A status named "${input.name}" already exists in this list`);
            }
            throw err;
        }
    }
    ...
    async updateById(input) {
        const existing = await this.statuses.findByIdInWorkspace(input.id, input.workspaceId);
        if (!existing) {
            this.logger.debug("statuses.update.not_found", {
                statusId: input.id,
                workspaceId: input.workspaceId,
            });
            throw errors_1.AppError.notFound("status.not_found", `Status ${input.id} does not exist`);
        }
        ...
        if (input.statusGroup !== undefined &&
            _shared_1.DONE_STATUS_GROUPS.has(input.statusGroup) !==
                _shared_1.DONE_STATUS_GROUPS.has(existing.statusGroup)) {
            const inUse = await this.statuses.countTasksUsingStatus(input.id);
            if (inUse > 0) {
                throw errors_1.AppError.conflict("status.in_use", `${inUse} task(s) use this status; move them off it before changing it across the done boundary`);
            }
        }
        let updated;
        try {
    ...
    async deleteById(input) {
        ...
        await this.db.transaction(async (tx) => {
            ...
            const groupIds = await this.statuses.lockGroupStatusIds(existing.scopeId, existing.statusGroup, tx);
            ...
            const taskCount = await this.statuses.countTasksUsingStatus(input.id, tx);
            if (taskCount > 0) {
                throw errors_1.AppError.conflict("status.in_use", `Status ${input.id} is still used by ${taskCount} task(s); move them to another status first`);
            }
            // 422 — refuse to empty a status_group (Board view needs ≥1 per group).
            if (groupIds.length <= 1) {
                throw errors_1.AppError.unprocessable("status.last_in_group", `Cannot delete the last "${existing.statusGroup}" status of this list`);
            }
            try {
                ...
                if (isReferencedError(err)) {
                    ...
                    throw errors_1.AppError.conflict("status.in_use", `Status ${input.id} is still used by a task; move it to another status first`);
                }
                throw err;
            }
        });
    ...
    async reorderForList(input) {
        ...
        return this.db.transaction(async (tx) => {
            const owned = new Set(await this.statuses.lockListStatusIds(input.listId, tx));
            // Validate the whole batch BEFORE any write — all-or-nothing.
            for (const item of input.items) {
                if (!owned.has(item.id)) {
                    throw errors_1.AppError.notFound("status.not_found", `Status ${item.id} is not part of list ${input.listId}`);
                }
            }
