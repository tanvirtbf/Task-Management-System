...
const formSerializer_1 = require("../serializers/formSerializer");
/**
 ...
 */
const decryptSubmissionData = (data) => {
    ...
    if (env !== null &&
        ...
        "authTag" in env) {
        try {
            return (0, encryption_1.decryptJSON)(envStr);
        }
        catch {
 ...
 * free text.
 */
const normalizeTaskAttr = (key, value) => {
    switch (key) {
        case "priority": {
...
class FormsService {
    ...
    notifications;
    logger;
    constructor(db, forms, fields, submissions, customFields, lists, taskWrites, notifications, logger) {
        this.db = db;
        this.forms = forms;
        this.fields = fields;
    ...
    async listByList(listId, workspaceId) {
        ...
        if (!list) {
            throw errors_1.AppError.notFound("list.not_found", `List ${listId} does not exist`);
        }
        const rows = await this.forms.listByList(listId, workspaceId);
        return Promise.all(rows.map((f) => this.hydrate(f)));
    }
    async get(formId, workspaceId) {
    ...
    async create(input) {
        ...
        if (list.archivedAt) {
            throw errors_1.AppError.conflict("list.archived", "Cannot create a form on an archived list");
        }
        const id = (0, utils_1.fakeId)("frm");
        ...
        for (let attempt = 1;; attempt += 1) {
            ...
            try {
                ...
                if (isDuplicateKeyError(err)) {
                    ...
                    if (attempt < SLUG_MAX_RETRIES)
                        continue;
                }
                throw err;
            }
        }
        const created = await this.forms.findByIdInWorkspace(id, input.workspaceId);
        if (!created)
            throw errors_1.AppError.internal("Form could not be loaded");
    ...
    async update(input) {
        ...
        const updated = await this.forms.findByIdInWorkspace(input.formId, input.workspaceId);
        if (!updated) {
            throw errors_1.AppError.notFound("form.not_found", `Form ${input.formId} does not exist`);
        }
        return this.hydrate(updated);
    }
    async delete(formId, workspaceId) {
    ...
    async addField(input) {
        await this.requireForm(input.formId, input.workspaceId);
        await this.validateFieldKey(input.fieldKind, input.fieldKey, input.workspaceId);
        const id = (0, utils_1.fakeId)("ffld");
        const position = input.position ?? (await this.fields.nextPosition(input.formId));
        const row = {
            id,
            formId: input.formId,
        ...
        try {
            ...
            if (isDuplicateKeyError(err)) {
                throw errors_1.AppError.conflict("form_field.duplicate", "This field is already on the form");
            }
            throw err;
        }
        const field = await this.fields.findByIdInWorkspace(id, input.workspaceId);
        if (!field)
            throw errors_1.AppError.internal("Field could not be loaded");
        return (0, formSerializer_1.toWireFormField)(field);
    }
    async updateField(input) {
        const existing = await this.fields.findByIdInWorkspace(input.fieldId, input.workspaceId);
        if (!existing) {
            throw errors_1.AppError.notFound("form_field.not_found", `Field ${input.fieldId} does not exist`);
        }
        const patch = {};
        ...
        if (input.isHidden !== undefined)
            patch.isHidden = input.isHidden;
        if (input.placeholder !== undefined)
            patch.placeholder = input.placeholder;
        if (input.helpText !== undefined)
            patch.helpText = input.helpText;
        if (input.defaultValue !== undefined)
            patch.defaultValue = input.defaultValue;
        await this.fields.update(input.fieldId, patch);
        const updated = await this.fields.findByIdInWorkspace(input.fieldId, input.workspaceId);
        if (!updated) {
            throw errors_1.AppError.notFound("form_field.not_found", `Field ${input.fieldId} does not exist`);
        }
        return (0, formSerializer_1.toWireFormField)(updated);
    }
    async deleteField(fieldId, workspaceId) {
        const existing = await this.fields.findByIdInWorkspace(fieldId, workspaceId);
        if (!existing) {
            throw errors_1.AppError.notFound("form_field.not_found", `Field ${fieldId} does not exist`);
        }
    ...
    async reorderFields(input) {
        ...
        if (foreign.length > 0) {
            throw errors_1.AppError.unprocessable("form_field.not_in_form", "One or more fields do not belong to this form", foreign.map((i) => ({
                field: "id",
    ...
    async listSubmissions(input) {
        ...
        const nextCursor = hasMore && last ? encodeCursor(last.internalId) : null;
        return {
            ...
            data: page.map((row) => (0, formSerializer_1.toWireFormSubmission)({
                ...row,
    ...
    // ─── public ───────────────────────────────────────────────────────────────
    async publicView(slug) {
        const resolved = await this.forms.resolveBySlug(slug);
        if (!resolved) {
            throw errors_1.AppError.notFound("form.not_found", "Form not found");
        }
        ...
        for (const key of cfKeys) {
            const cf = await this.customFields.findByIdInWorkspace(key, workspaceId);
            if (cf) {
                typeByKey.set(key, cf.type);
                configByKey.set(key, (cf.config ?? {}));
            }
        }
        const dropdownKeys = cfKeys.filter((k) => typeByKey.get(k) === "dropdown");
        const rawOptions = await this.customFields.optionsByFieldIds(dropdownKeys);
        const optionsByKey = new Map();
        for (const [k, opts] of rawOptions) {
            optionsByKey.set(k, opts.map((o) => ({ id: o.id, label: o.label, color: o.color })));
        }
        return (0, formSerializer_1.toPublicForm)(form, fields, {
            typeByKey,
            configByKey,
            optionsByKey,
        });
    ...
    async submit(input) {
        ...
        if (!(0, encryption_1.encryptionReady)()) {
            throw new errors_1.AppError(503, "form.encryption_unavailable", "Form intake is temporarily unavailable (encryption not configured)");
        }
        ...
        if (!form.isPublic || settings.submission_open === false) {
            throw errors_1.AppError.forbidden("form.submission_closed", "This form is not accepting submissions");
        }
        const fields = await this.fields.listByForm(form.id);
        const details = [];
        const taskAttrs = {};
        ...
        for (const field of fields) {
            ...
            if (field.fieldKind === "task_attr") {
                ...
                const cf = await this.customFields.findByIdInWorkspace(field.fieldKey, workspaceId);
                if (!cf) {
                    details.push({
                        field: field.fieldKey,
        ...
        const submissionId = (0, utils_1.fakeId)("fsub");
        const submitterEmail = typeof input.data.email === "string" ? input.data.email : null;
        const now = new Date();
        const expiresAt = new Date(now.getTime() + 90 * 24 * 60 * 60 * 1000); // 90 days
        const encryptedData = (0, encryption_1.encryptJSON)(input.data);
        await this.db.transaction(async (tx) => {
            for (const cv of cfValues) {
                await this.customFields.upsertValue(task.id, cv.customFieldId, cv.value, form.createdBy, tx);
            }
        ...
        return {
            ...
            task_id: task.id,
            message: typeof settings.success_message === "string"
                ? settings.success_message
                : "Thank you",
        };
    }
    // ─── helpers ─────────────────────────────────────────────────────────────
    async requireForm(formId, workspaceId) {
        const form = await this.forms.findByIdInWorkspace(formId, workspaceId);
        if (!form) {
            throw errors_1.AppError.notFound("form.not_found", `Form ${formId} does not exist`);
        }
        return form;
    }
    async hydrate(form) {
        const fields = await this.fields.listByForm(form.id);
        return (0, formSerializer_1.toWireForm)(form, fields);
    }
    /** Reject a field whose key cannot target a task attr / custom field. */
    async validateFieldKey(fieldKind, fieldKey, workspaceId) {
        if (fieldKind === "task_attr") {
            if (!TASK_ATTR_KEYS.has(fieldKey)) {
                throw errors_1.AppError.unprocessable("form.invalid_field_key", `task_attr field_key must be one of: ${[...TASK_ATTR_KEYS].join(", ")}`, [{ field: "field_key", issue: "is not a task attribute" }]);
            }
            return;
        }
        const cf = await this.customFields.findByIdInWorkspace(fieldKey, workspaceId);
        if (!cf) {
            throw errors_1.AppError.unprocessable("form.invalid_field_key", `${fieldKey} is not a custom field in this workspace`, [{ field: "field_key", issue: "is not a custom field" }]);
        }
    }
    /** Validate a submitted custom-field value envelope per type. Returns an issue string or null. */
    async validateCustomValue(cf, value) {
        const v = value;
        ...
        switch (cf.type) {
            ...
            case "dropdown": {
                ...
                const ok = await this.customFields.optionExists(cf.id, v.option_id);
...
const clampLimit = (raw) => {
    if (raw === undefined)
        return SUBMISSIONS_DEFAULT_LIMIT;
    if (raw < 1)
        return 1;
    if (raw > SUBMISSIONS_MAX_LIMIT)
        return SUBMISSIONS_MAX_LIMIT;
