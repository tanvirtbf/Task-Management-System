...
const parseTypes = (raw) => {
    ...
    if (tokens.length === 0)
        return null;
    const valid = tokens.filter((t) => VALID_TYPES.includes(t));
...
class SearchService {
    ...
    async search(input) {
        ...
        const want = (t) => requested === null || requested.has(t);
        ...
        const [taskRows, listRows, spaceRows, userRows, commentRows] = await Promise.all([
            want("task")
        ...
        const users = userRows.map(userSerializer_1.toWireUser);
