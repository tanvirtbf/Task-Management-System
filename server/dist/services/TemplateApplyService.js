...
exports.TemplateApplyService = void 0;
...
class TemplateApplyService {
    ...
    membership;
    activity;
    constructor(db, templates, taskTypes, tags, lists, statuses, tasks, membership, activity) {
        this.db = db;
    ...
    async apply(input) {
        ...
        if (!template) {
            throw errors_1.AppError.notFound("template.not_found", `Template ${input.templateId} does not exist`);
        }
        ...
        if (list.archivedAt) {
            throw errors_1.AppError.conflict("list.archived", "Cannot apply a template to an archived list");
        }
        ...
        const items = Array.isArray(structure.checklistItems)
            ...
            : [];
        if (items.length === 0) {
            throw errors_1.AppError.unprocessable("template.empty_structure", "Template has no checklist items to materialise");
        }
        ...
        if (!taskTypeId) {
            throw errors_1.AppError.unprocessable("template.invalid_task_type", "Template has no task type and the target list has no default");
        }
        const taskType = await this.taskTypes.findByIdInWorkspace(taskTypeId, input.workspaceId);
        if (!taskType) {
            throw errors_1.AppError.unprocessable("template.invalid_task_type", `Task type ${taskTypeId} does not exist in this workspace`);
        }
        ...
        const tagIds = Array.isArray(structure.tags)
            ...
            : [];
        if (tagIds.length > 0) {
            const found = await this.tags.findIdsInWorkspace(tagIds, input.workspaceId);
        ...
        for (let attempt = 1;; attempt += 1) {
            ...
            try {
                await this.db.transaction(async (tx) => {
                    ...
                    await tx.insert(schema_1.checklists).values({
                        id: checklistId,
                        taskId,
                        name: checklistName,
                        position: 0,
        ...
        const created = await this.tasks.findByIdOrCustomIdInWorkspace(taskId, input.workspaceId);
...
exports.TemplateApplyService = TemplateApplyService;
