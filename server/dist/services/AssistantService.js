...
class AssistantService {
    ...
    maxTokens;
    logger;
    constructor(client, model, maxTokens, logger) {
        this.client = client;
        this.model = model;
    ...
    async ask(history, message, opts = {}) {
        ...
        const messages = (0, buildMessages_1.buildMessages)(history, message, opts.callerBlock, opts.today);
        ...
        for (let round = 0; round < maxRounds; round++) {
            ...
            if (!opts.tools || toolCalls.length === 0) {
                ...
                if (!reply) {
                    throw new errors_1.AppError(502, "assistant.empty_reply", "The assistant did not return a response. Please try again.");
                }
                return reply;
            }
            await this.appendToolRound(messages, toolCalls, choice?.content ?? null, opts.tools.execute);
        }
     ...
     */
    async appendToolRound(messages, toolCalls, content, execute) {
        messages.push({
            role: "assistant",
    ...
    async streamReply(history, message, opts) {
        ...
        for (let round = 0; round < maxRounds; round++) {
            ...
            if (toolCalls.length === 0 || !opts.tools)
                return;
            ...
            await this.appendToolRound(messages, toolCalls, streamedText, opts.tools.execute);
        }
    }
    /**
    ...
    toAppError(err) {
        ...
        if (status === 429) {
            return new errors_1.AppError(503, "assistant.rate_limited", "The assistant is busy right now. Please try again in a moment.");
        }
        // Network timeout / connection failure.
        if (name.includes("Timeout") || name.includes("Connection")) {
            return new errors_1.AppError(504, "assistant.timeout", "The assistant took too long to respond. Please try again.");
        }
        // Auth, 5xx, or anything else.
        return new errors_1.AppError(502, "assistant.upstream_error", "The assistant is temporarily unavailable. Please try again.");
    }
}
