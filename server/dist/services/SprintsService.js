...
class SprintsService {
    ...
    async create(input) {
        ...
        const clash = await this.sprints.findOverlapping(input.workspaceId, toDateOnly(input.startDate), toDateOnly(input.endDate));
        if (clash) {
            throw errors_1.AppError.conflict("sprint.overlap", `Dates overlap sprint "${clash.name}" (${clash.id})`);
        }
    ...
    async update(input) {
        ...
        const row = await this.db.transaction(async (tx) => {
            ...
            if (fields.startDate !== undefined ||
                ...
                const clash = await this.sprints.findOverlapping(input.workspaceId, toDateOnly(effStart), toDateOnly(effEnd), current.id, tx);
                if (clash) {
                    throw errors_1.AppError.conflict("sprint.overlap", `Dates overlap sprint "${clash.name}" (${clash.id})`);
                }
            }
    ...
    async remove(input) {
        ...
        return this.db.transaction(async (tx) => {
            ...
            if (current.status === "active") {
                throw errors_1.AppError.conflict("sprint.active_immutable", "An active sprint cannot be deleted — close it first");
            }
            const detached = await this.sprints.countTasksInSprint(id, workspaceId, tx);
