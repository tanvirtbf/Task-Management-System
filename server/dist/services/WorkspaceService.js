...
class WorkspaceService {
    ...
    workspaces;
    activity;
    constructor(db, workspaces, activity) {
        this.db = db;
    ...
    async getWorkspace(workspaceId) {
        ...
        if (!workspace) {
            throw errors_1.AppError.notFound("workspace.not_found", `Workspace ${workspaceId} does not exist`);
        }
        return workspace;
    }
    /**
    ...
    async updateWorkspace(input) {
        ...
        return this.db.transaction(async (tx) => {
            ...
            const current = await this.workspaces.findByIdForUpdate(workspaceId, tx);
            if (!current) {
                throw errors_1.AppError.notFound("workspace.not_found", `Workspace ${workspaceId} does not exist`);
            }
            ...
            if (start >= end) {
                throw errors_1.AppError.unprocessable("workspace.invalid_business_hours", "business_hours_start must be earlier than business_hours_end");
            }
            const updated = await this.workspaces.update(workspaceId, patch, tx);
