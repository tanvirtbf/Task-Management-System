...
const seed = async () => {
    ...
    const [memberRole] = await db
        ...
        .from(S.roles)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(S.roles.workspaceId, ws), (0, drizzle_orm_1.eq)(S.roles.roleKey, "member")));
    ...
    const existingPairs = new Set((await db
        .select({
        userId: S.userRoleGrants.userId,
        scopeId: S.userRoleGrants.scopeId,
    })
        .from(S.userRoleGrants)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(S.userRoleGrants.workspaceId, ws), (0, drizzle_orm_1.eq)(S.userRoleGrants.scopeType, "space")))).map((r) => `${r.userId}|${r.scopeId}`));
    for (const [idxStr, teams] of Object.entries(roster)) {
        const idx = Number(idxStr);
        for (const key of teams) {
            if (existingPairs.has(`${U[idx]}|${SP[key]}`))
                continue;
            await db.insert(S.userRoleGrants).values({
                id: (0, utils_1.fakeId)("ur"),
                workspaceId: ws,
                userId: U[idx],
        ...
        await db
            ...
            .where((0, drizzle_orm_1.eq)(S.users.id, U[idx]));
    }
    logger_1.default.info("Team roster seeded (home teams + memberships)");
    ...
    const mgRoles = await db
        .select({ id: S.roles.id })
        .from(S.roles)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(S.roles.workspaceId, ws), (0, drizzle_orm_1.inArray)(S.roles.roleKey, ["member", "guest"])));
    const mgIds = mgRoles.map((r) => r.id);
    await db
        .update(S.rolePermissions)
        .set({ scope: "space" })
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.inArray)(S.rolePermissions.roleId, mgIds), (0, drizzle_orm_1.eq)(S.rolePermissions.permissionKey, "space.view")));
    await db
        .update(S.rolePermissions)
        .set({ scope: "own" })
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.inArray)(S.rolePermissions.roleId, mgIds), (0, drizzle_orm_1.eq)(S.rolePermissions.permissionKey, "task.view")));
    await db
        .update(S.rolePermissions)
        .set({ scope: "own" })
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(S.rolePermissions.roleId, memberRole.id), (0, drizzle_orm_1.inArray)(S.rolePermissions.permissionKey, [
        "task.edit",
        "task.archive",
        "task.delete",
    ])));
    await db
        .update(S.workspaces)
        .set({ permissionsVersion: (0, drizzle_orm_1.sql) `${S.workspaces.permissionsVersion} + 1` })
        .where((0, drizzle_orm_1.eq)(S.workspaces.id, ws));
    logger_1.default.info("Visibility switch + edit rights applied (019/020 semantics)");
    ...
    if (demoReqTask) {
        const reqId = (0, utils_1.fakeId)("areq");
        const now = new Date();
        ...
        await db.insert(S.taskAssignmentRequests).values({
            ...
            status: "pending",
            requestNote: "Discount codes need to go into the system before the Eid sale — can you take this?",
        ...
        logger_1.default.info("Demo cross-team assignment request seeded (pending, mid-query)");
    }
