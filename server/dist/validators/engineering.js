...
exports.createPostmortemValidator = exports.getPostmortemValidator = exports.reportBugValidator = void 0;
 ...
 * bug-severity enum.
 */
exports.reportBugValidator = (0, express_validator_1.checkSchema)({
    steps: {
        in: ["body"],
    ...
    severity: {
        ...
        isIn: {
            ...
            errorMessage: `severity must be one of: ${_shared_1.bugSeverities.join(", ")}`,
        },
    },
    reporter_team: {
        in: ["body"],
...
exports.getPostmortemValidator = (0, express_validator_1.checkSchema)({
    id: {
        in: ["params"],
        trim: true,
...
exports.createPostmortemValidator = (0, express_validator_1.checkSchema)({
    id: {
        in: ["params"],
        trim: true,
        notEmpty: { errorMessage: "id is required" },
        ...
        custom: {
            options: (value) => {
                if (typeof value !== "object" ||
                    value === null ||
                    Array.isArray(value)) {
                    throw new Error("items must be an object of checklist label → boolean");
                }
                const entries = Object.entries(value);
                if (entries.length > POSTMORTEM_ITEMS_MAX) {
                    throw new Error(`items has too many entries (max ${POSTMORTEM_ITEMS_MAX})`);
                }
                for (const [label, checked] of entries) {
                    if (label.length === 0 || label.length > ITEM_LABEL_MAX) {
                        throw new Error(`each item label must be 1-${ITEM_LABEL_MAX} characters`);
                    }
