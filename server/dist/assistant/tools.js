...
exports.makeAssistantToolExecutor = makeAssistantToolExecutor;
...
exports.ASSISTANT_TOOL_DEFS = [
    ...
    },
    {
        ...
        function: {
            ...
            description: "Everything about ONE task the user can see — status, assignees, due date, priority, list/space, checklist progress, review verdict. Use for 'X task er ki obostha?', 'checklist koto% hoyeche?', 'kake deya ache?'. Accepts the task's name, its id, or its custom id. If it answers not_found, do NOT conclude the task exists somewhere hidden — say it was not found or is not visible to them.",
            parameters: {
                type: "object",
                properties: {
                    task: {
                        type: "string",
                        description: "The task's name as the user said it, or its id / custom id",
                    },
                },
                required: ["task"],
                additionalProperties: false,
    ...
    },
    {
        ...
        function: {
            ...
            description: "Bugs/complaints past their SLA deadline and not done, newest breach first — only ones the user can see. Use for 'kono task SLA miss korse?', 'deadline par hoye gese kongula?'.",
            parameters: {
                type: "object",
                properties: {},
                additionalProperties: false,
            ...
            parameters: {
                ...
                properties: {
                    ...
                    assignee_names: {
                        type: "array",
                        items: { type: "string" },
                        description: 'Optional people to assign, by name as the user said them. When the user wants the task assigned to THEMSELVES ("assign it to me"), pass the literal string "@me" — never guess their name.',
                    },
                },
 ...
 * corrected arguments.
 */
function makeAssistantToolExecutor(ctx, services) {
    const createdThisRequest = new Map();
...
async function executeAssistantTool(name, args, ctx, services) {
    switch (name) {
        ...
        case "get_my_tasks": {
            const bucket = MY_TASK_BUCKETS.includes(args.bucket)
                ? args.bucket
                : "open";
        ...
        case "get_sla_breaches": {
            ...
            return {
                ...
                breaches: shown.map((b) => ({
                    ...
                    assignees: b.assignees.map((a) => `${a.first_name} ${a.last_name}`.trim()),
                })),
            };
        }
        case "create_task": {
 ...
 */
async function taskDetailsTool(args, ctx, services) {
    const wanted = typeof args.task === "string" ? args.task.trim() : "";
    if (!wanted)
        return { error: "Which task? Ask the user for its name." };
    const NOT_FOUND = {
        error: "not_found",
        code: "task.not_found",
    ...
    let row = await services.tasks.findDetailInWorkspace(wanted, ctx.workspaceId);
    // 2) Otherwise resolve the NAME through the caller's own search.
    if (!row) {
        const byName = async (q) => (await services.search.search({
            workspaceId: ctx.workspaceId,
            role: ctx.role,
            q,
            limit: 8,
        ...
        if (hits.length === 0) {
            ...
            if (loose)
                hits = await byName(loose);
        }
        const pick = exactMatch(hits, wanted) ?? (hits.length === 1 ? hits[0] : null);
        if (!pick) {
        ...
        row = await services.tasks.findDetailInWorkspace(pick.id, ctx.workspaceId);
        if (!row)
            return NOT_FOUND;
    }
    ...
    const people = ids.length > 0
        ? await services.users.findManyByIdsInWorkspace(ids, ctx.workspaceId)
        : [];
 ...
 */
const denied = (key) => ({
    error: (0, can_1.denyMessage)(key, "no_grant"),
    code: (0, can_1.permissionErrorCode)(key),
    permission: key,
    reason: "no_grant",
});
/**
 ...
 */
const resolvePerson = async (wanted, ctx, services) => {
    if (wanted.trim().toLowerCase() === "@me") {
        const me = await services.users.findByIdInWorkspace(ctx.userId, ctx.workspaceId);
        return {
            person: {
                id: ctx.userId,
 ...
 */
async function peopleTool(args, ctx, services) {
    ...
    if (action === "my_teams") {
        const mine = new Set(await services.userRoles.spaceIdsForUser(ctx.userId, ctx.workspaceId));
        ...
        const heads = headIds.length > 0
            ? await services.users.findManyByIdsInWorkspace(headIds, ctx.workspaceId)
            : [];
        const headName = new Map(heads.map((h) => [h.id, `${h.firstName} ${h.lastName}`.trim()]));
        return {
            count: teams.length,
            teams: teams.map((t) => ({
                team: t.name,
    ...
    if (action === "team_roster") {
        ...
        const team = exactMatch(named, teamName) ??
            (named.filter((s) => normalizeName(s.name).includes(normalizeName(teamName))).length === 1
                ? named.find((s) => normalizeName(s.name).includes(normalizeName(teamName)))
                : null);
        if (!team) {
            const close = named.filter((s) => normalizeName(s.name).includes(normalizeName(teamName)));
        ...
        const assignments = await services.userRoles.listBySpace(team.id, ctx.workspaceId);
    ...
    if (action === "person_workload") {
        const open = await services.tasks.countOpenAssignedVisible(person.id, ctx.workspaceId);
        return {
            person: person.name,
    ...
    const theirSpaceIds = await services.userRoles.spaceIdsForUser(person.id, ctx.workspaceId);
    const primaryId = await services.users.primarySpaceIdOf(person.id, ctx.workspaceId);
...
async function reportStatusTool(args, ctx, services) {
    ...
    const seesAll = isAdminish && (0, can_1.holds)(actor, "report.view");
...
async function createTaskTool(args, ctx, services) {
    ...
    const rawNames = Array.isArray(args.assignee_names)
        ...
        : [];
    const onlySelf = rawNames.every((n) => n.trim().toLowerCase() === "@me");
