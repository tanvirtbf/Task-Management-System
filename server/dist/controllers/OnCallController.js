...
exports.OnCallController = void 0;
const onCallSerializer_1 = require("../serializers/onCallSerializer");
/**
...
class OnCallController {
    ...
    async current(req, res, next) {
        try {
            ...
            res.status(200).json(shift ? (0, onCallSerializer_1.toWireOnCallShift)(shift) : null);
        }
        catch (err) {
            next(err);
        }
    ...
    async schedule(req, res, next) {
        try {
            ...
            res.status(200).json({
                data: rows.map(onCallSerializer_1.toWireOnCallShift),
                pagination: {
                    next_cursor: null,
                    has_more: false,
    ...
    async set(req, res, next) {
        try {
            ...
            res.status(200).json((0, onCallSerializer_1.toWireOnCallShift)(shift));
        }
        catch (err) {
            next(err);
        }
...
exports.OnCallController = OnCallController;
