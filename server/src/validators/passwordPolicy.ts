 ...
 */
export const newPasswordSchema: ParamSchema = {
    in: ["body"],
    notEmpty: { errorMessage: "New password is required" },
    isString: { errorMessage: "New password must be a string", bail: true },
    custom: {
        options: (value: unknown): boolean => passwordPolicyError(value) === null,
        errorMessage: (value: unknown): string =>
            passwordPolicyError(value) ?? "New password is not acceptable",
    },
};
