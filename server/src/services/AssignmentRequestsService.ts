...
import {
    AssignmentRequestsRepo,
    type AssignmentRequestEventRow,
...
import type { UserListRow } from "../repositories/UsersRepo";
...
const notifTitle = (prefix: string, taskName: string): string => {
...
/** One (task, person) assignment the gate classifies. */
export interface GatePair {
    taskId: string;
    /** The space owning the task — supplied by the caller (list/spaceInfo). */
    spaceId: string;
    targetUserId: string;
    /** Denormalised into the request notification titles. */
    taskName: string;
}

export interface GateSplit {
    /** Per task: the targets to assign directly (same-team / exempt / all-reach). */
    directByTask: Map<string, string[]>;
    /** The pairs that must become pending requests instead. */
    gated: GatePair[];
    /** Heads of each gated target's teams — request-notification fanout (Q2). */
...
export interface AnswerDateChange {
    taskId: string;
    workspaceId: string;
    actorId: string;
    actorRole: string;
...
export interface DueDateChanger {
    apply(input: AnswerDateChange): Promise<void>;
}

/** A request hydrated for the wire (serializer input). */
export interface RequestDetail {
    request: AssignmentRequestRow;
    task: RequestTaskSnapshot | null;
    events: AssignmentRequestEventRow[];
    usersById: Map<string, UserListRow>;
}

export type RequestBox = "received" | "sent" | "team";

interface ActorInput {
    workspaceId: string;
    actorId: string;
    /** JWT role claim — `liveLegacyRole` upgrades it to the live row when possible. */
    actorRole: string;
}
...
export class AssignmentRequestsService {
    constructor(
        ...
        private repo: AssignmentRequestsRepo,
        ...
        private dueDateChanger?: DueDateChanger,
    ) {}
    ...
    async splitByApproval(input: {
        ...
        const directByTask = new Map<string, string[]>();
        const gated: GatePair[] = [];
        const pushDirect = (p: GatePair): void => {
        ...
        if (input.exempt || input.pairs.length === 0) {
            for (const p of input.pairs) pushDirect(p);
        ...
        const reachAll = new Map<string, boolean>();
        for (const uid of new Set(input.pairs.map((p) => p.targetUserId))) {
            ...
            const actor = await this.policy.resolveActor(
                uid,
                input.workspaceId,
            );
            reachAll.set(uid, actor ? entryFor(actor, "task.view").all : true);
        }

        // Q11 — one membership probe per distinct (user, space) pair.
        const membership = new Map<string, boolean>();
        for (const p of input.pairs) {
            if (!p.spaceId || reachAll.get(p.targetUserId)) continue;
            const key = `${p.targetUserId}|${p.spaceId}`;
            if (membership.has(key)) continue;
            membership.set(
                key,
                await this.userRoles.hasSpaceMembership(
                    p.targetUserId,
                    p.spaceId,
                    input.workspaceId,
                ),
        ...
        const headsByTarget =
            gated.length > 0
                ? await this.repo.headsOfUserSpaces(
                      [...new Set(gated.map((p) => p.targetUserId))],
                      input.workspaceId,
                  )
    ...
    async createRequestsInTx(
        ...
        const createdIds: string[] = fresh.map(() => fakeId("areq"));
        await this.repo.insertRequests(
            fresh.map((pair, i) => ({
                id: createdIds[i],
    ...
    async listFor(
        input: ActorInput & { box: RequestBox; onlyPending: boolean },
    ): Promise<RequestDetail[]> {
        const { workspaceId, actorId } = input;
        let rows: AssignmentRequestRow[];
        if (input.box === "received") {
            rows = await this.repo.listByTarget(
                actorId,
                workspaceId,
                input.onlyPending,
                LIST_LIMIT,
            ...
            rows = await this.repo.listByRequester(
                actorId,
                workspaceId,
                input.onlyPending,
                LIST_LIMIT,
            ...
            rows = await this.repo.listByHeadOf(
                actorId,
                workspaceId,
                input.onlyPending,
                LIST_LIMIT,
    ...
    private async hydrate(
        rows: AssignmentRequestRow[],
        workspaceId: string,
    ): Promise<RequestDetail[]> {
        if (rows.length === 0) return [];
        const [snapshots, events] = await Promise.all([
            this.repo.taskSnapshotByIds(
                [...new Set(rows.map((r) => r.taskId))],
                workspaceId,
            ),
        ...
        for (const list of events.values()) {
            for (const e of list) if (e.actorId) userIds.add(e.actorId);
        }
        const users = await this.users.findManyByIdsInWorkspace(
            [...userIds],
            workspaceId,
        );
     ...
     */
    async accept(
        input: ActorInput & { requestId: string; note?: string | null },
    ): Promise<RequestDetail> {
        ...
        const active = await this.users.findActiveIdsInWorkspace(
            [r.targetUserId],
            input.workspaceId,
        );
        if (!active.has(r.targetUserId)) {
            throw AppError.conflict(
                "request.user_inactive",
                "The requested person is no longer an active member",
            );
        }
        ...
        await this.db.transaction(async (tx) => {
            ...
            const live = await this.repo.taskSnapshotByIds(
                [r.taskId],
                input.workspaceId,
                tx,
            );
            if (live.get(r.taskId)?.archivedAt) {
                throw AppError.conflict(
                    "request.task_archived",
                    "The task has been archived; unarchive it before accepting",
                );
            }
            ...
            const existing = await this.membership.getAssigneeIds(
                r.taskId,
                tx,
            );
            ...
            if (r.requestedBy !== input.actorId) {
                rows.push({
                    ...
                    type: "assignment_request_decided" as const,
                    entityType: "task" as const,
                    entityId: r.taskId,
                    actorId: input.actorId,
                    title: notifTitle(
    ...
    async query(
        ...
        const now = new Date();
        await this.db.transaction(async (tx) => {
            const recorded = await this.repo.recordQuery(
                r.id,
                {
            ...
            if (!recorded) throw await this.staleError(r.id, tx, now);
            await this.repo.insertEvents(
                [
                    {
                        id: fakeId("arev"),
    ...
    async answer(
        ...
        if (r.requestedBy !== input.actorId) {
            throw AppError.forbidden(
                "request.not_requester",
                "Only the requester may answer a query on this request",
            );
        }
        this.assertPendingRead(r);
        ...
        if (input.dueDate !== undefined) {
            if (!this.dueDateChanger) {
                throw AppError.internal(
                    "Assignment answers are not wired for date changes here",
                );
            }
            if (snapshot.archivedAt) {
                throw AppError.conflict(
                    "request.task_archived",
                    "The task has been archived; unarchive it before answering",
                );
            }
        ...
        await this.db.transaction(async (tx) => {
            ...
            if (!touched) throw await this.staleError(r.id, tx, now);
            await this.repo.insertEvents(
                [
                    {
                        id: fakeId("arev"),
            ...
            const recipients = new Set<string>([r.targetUserId]);
            if (lastQueryActor) recipients.add(lastQueryActor);
            recipients.delete(input.actorId);
            await this.notifications.createMany(
                [...recipients].map((userId) => ({
                    userId,
                    type: "assignment_query" as const,
                    entityType: "task" as const,
                    entityId: r.taskId,
                    actorId: input.actorId,
                    title: notifTitle(
    ...
    async cancel(
        input: ActorInput & { requestId: string },
    ): Promise<RequestDetail> {
        const r = await this.requireRequest(input.requestId, input.workspaceId);
        await this.assertParty(r, input);
        ...
        if (r.requestedBy !== input.actorId && !isAdmin) {
            throw AppError.forbidden(
                "request.not_requester",
                "Only the requester (or an admin) may withdraw this request",
            );
        }
        this.assertPendingRead(r);
        const snapshot = await this.requireTask(r);
    ...
    async expireDue(input: {
        ...
        for (const row of due) {
            const didExpire = await this.db.transaction(async (tx) => {
                const claimed = await this.repo.claimDecision(
                    row.id,
                    {
                        to: "expired",
                        decidedBy: null,
    ...
    ): Promise<RequestTaskSnapshot> {
        const map = await this.repo.taskSnapshotByIds(
            [r.taskId],
            r.workspaceId,
        );
     ...
     */
    private async assertDecider(
        r: AssignmentRequestRow,
        input: ActorInput,
    ): Promise<void> {
        const { isDecider } = await this.assertParty(r, input);
        if (
            input.actorId === r.requestedBy &&
            input.actorId !== r.targetUserId
        ) {
            throw AppError.forbidden(
                "request.not_decider",
        ...
        if (!isDecider) {
            throw AppError.forbidden(
                "request.not_decider",
                "Only the requested person, their team head or an admin may decide this request",
            );
        }
    }

    /** Friendly pre-checks on the row as read (the claim is the authority). */
    private assertPendingRead(r: AssignmentRequestRow): void {
        if (r.status !== "pending") {
            throw AppError.conflict(
                "request.already_decided",
                `This request was already ${r.status}`,
            );
        }
    }
    ...
    private async claimOrThrow(
        ...
        tx: DbExecutor,
        input: {
            to: "accepted" | "declined" | "cancelled";
            decidedBy: string;
            now: Date;
        ...
        if (!claimed) throw await this.staleError(r.id, tx, input.now);
    }

    /** Why did the conditional UPDATE miss? Decided vs expired, precisely. */
    private async staleError(
        id: string,
        tx: DbExecutor,
        now: Date,
    ): Promise<AppError> {
        ...
        if (fresh && fresh.status !== "pending") {
            return AppError.conflict(
                "request.already_decided",
                `This request was already ${fresh.status}`,
            );
        }
...
let gateInstance: AssignmentRequestsService | null = null;

export const assignmentGate = (): AssignmentRequestsService => {
    if (!gateInstance) {
        const db = getDb();
        gateInstance = new AssignmentRequestsService(
            db,
...
export const resetAssignmentGate = (): void => {
    gateInstance = null;
};
