...
import {
    assignmentGate,
    type GateSplit,
    type RequestCreatedDelivery,
} from "./AssignmentRequestsService";
...
const hasSlaPolicy = (typeName: string): boolean => {
...
const isBugType = (typeName: string): boolean =>
    typeName.trim().toLowerCase() === "bug";
 ...
 */
const assertEngFieldsAllowed = (
    taskType: { name: string; isDevType: boolean },
    supplied: EngFieldInput,
): void => {
    if (!taskType.isDevType) {
        const offending = DEV_ONLY_FIELDS.filter(
            ([, prop]) => supplied[prop] !== undefined && supplied[prop] !== null,
        );
        if (offending.length > 0) {
            throw AppError.unprocessable(
                "task.not_dev_type",
                `Task type "${taskType.name}" is not a dev type; it cannot carry ${offending
                    .map(([wire]) => wire)
                    .join(", ")}`,
                offending.map(([wire]) => ({
                    field: wire,
    ...
    if (
        supplied.bugSeverity !== undefined &&
        supplied.bugSeverity !== null &&
        !isBugType(taskType.name)
    ) {
        throw AppError.unprocessable(
            "task.severity_requires_bug_type",
            `Task type "${taskType.name}" is not a Bug type; bug_severity does not apply`,
            [{ field: "bug_severity", issue: "requires a Bug task type" }],
        );
    }
};
...
const computeSlaDueAt = (
    ...
    const hours = (n: number): Date =>
        cal ? addBusinessMs(now, n * HOUR_MS, cal) : new Date(now.getTime() + n * HOUR_MS);
    const days = (n: number): Date =>
        cal ? addBusinessDays(now, n, cal) : new Date(now.getTime() + n * DAY_MS);
...
const refuseTimeWithoutDate = (field: "due" | "start"): never => {
    throw AppError.unprocessable(
        "task.time_without_date",
        `${field}_time needs a ${field}_date`,
        [
            {
                field: `${field}_time`,
 ...
 */
const resulting = <P, C>(patched: P | undefined, current: C): P | C =>
 ...
 */
const storedDateYmd = (d: Date): string => {
...
export interface BulkResult {
    ...
    tasks: WireTask[];
    /**
     * Team-access P8 (Q8): how many (task, user) assignments became PENDING
     * APPROVAL requests instead of assignee rows — the toolbar reports
     * "N assigned, M pending approval" rather than silently succeeding.
     * 0 whenever the gate is dormant or the patch adds no assignees.
     */
...
export interface CreateTaskInput {
    ...
    /**
     * Team-access P8, Q7 — INTERNAL ONLY (never mapped from an HTTP body):
     * skips the cross-team approval gate on the initial assignees. Set solely
     * by `EngineeringService.reportBug` for the S0/S1 on-call auto-assign —
     * an incident page that waits for an accept is not a page.
     */
    exemptAssignmentApproval?: boolean;
}
...
const scalarChanges = (
    ...
    const wire = (k: string): string =>
        k.replace(/[A-Z]/g, (c) => `_${c.toLowerCase()}`);
    const norm = (v: unknown): unknown => {
        if (v instanceof Date) return storedDateYmd(v);
    ...
    for (const [key, to] of Object.entries(patch)) {
        if (to === undefined || NOT_SCALAR.has(key)) continue;
...
export class TaskWriteService {
     ...
     */
    private async businessCalendar(
        workspaceId: string,
    ...
    async create(input: CreateTaskInput): Promise<WireTask> {
        ...
        await assertScoped("task.create", {
        ...
        if (input.reviewerId !== undefined && input.reviewerId !== null) {
            ...
            if (!valid.has(input.reviewerId)) {
                throw AppError.unprocessable(
                    ...
                    `${input.reviewerId} is not an active member of this workspace`,
                    [
                        {
                            field: "reviewer_id",
        ...
        let requestDeliveries: RequestCreatedDelivery[] = [];
        ...
        let approvalSplit: GateSplit | null = null;
        if (assignees.length > 0) {
            approvalSplit = await assignmentGate().splitByApproval({
            ...
            try {
                await this.db.transaction(async (tx) => {
                    ...
                    if (directAssignees.length > 0) {
                        ...
                        await this.membership.addWatchers(
                            taskId,
                            directAssignees,
                            tx,
                        );
                    ...
                    const recipients = directAssignees.filter(
                        (id) => id !== input.actorId,
                    );
                    ...
                    if (input.parentTaskId) {
                        await this.tasks.recomputeSubtaskCounters(
                            input.parentTaskId,
                            tx,
                        );
                    }
        ...
        //    on accept.
        const emailRecipients = directAssignees.filter(
            (id) => id !== input.actorId,
        );
    ...
    async update(input: UpdateTaskInput): Promise<WireTask> {
        ...
        let effectiveType: { name: string; isDevType: boolean } | null = null;
        ...
        const suppliesEngValue =
            DEV_ONLY_FIELDS.some(
                ([, prop]) => p[prop] !== undefined && p[prop] !== null,
            ) ||
            (p.bugSeverity !== undefined && p.bugSeverity !== null);
        if (suppliesEngValue && !effectiveType) {
            effectiveType = await this.taskTypes.findByIdInWorkspace(
                current.taskTypeId,
                input.workspaceId,
            );
        }
        if (effectiveType) assertEngFieldsAllowed(effectiveType, p);
        ...
        if (timeWithoutDate(resulting(p.dueDate, current.dueDate), p.dueTime))
            refuseTimeWithoutDate("due");
        if (
            timeWithoutDate(resulting(p.startDate, current.startDate), p.startTime)
        )
            refuseTimeWithoutDate("start");
        ...
        if (
            ...
            DONE_GROUPS.has(
                listStatuses.find((s) => s.id === p.statusId)?.statusGroup ?? "",
            )
        ) {
            ...
            if (blockers > 0) {
                throw AppError.conflict(
                    ...
                    `This task is blocked by ${blockers} open task(s) — complete or unlink them first`,
                );
            }
        }
        ...
        try {
            await this.db.transaction(async (tx) => {
                ...
                if (
                    ...
                    const statusName =
                        listStatuses.find((s) => s.id === p.statusId)?.name ??
                        "a new status";
                    const recipients = [
                        ...new Set([
    ...
    async del(input: DeleteTaskInput): Promise<void> {
        ...
        if (input.hard) {
            ...
            const granted =
                legacyAdmin &&
                holds(await currentActor(), "task.delete_hard");
            if (!granted) {
        ...
        if (input.hard) {
            await this.db.transaction(async (tx) => {
                ...
                const keys = await this.attachmentsRepo.storageKeysByTasks(
                    subtree,
                    tx,
                );
    ...
    private async assertTaskScope(
        ...
        return assertTaskScoped(key, task, this.tasks);
    }
    ...
    async myWork(
        ...
        const today = todayInZone(ws?.timezone ?? "Asia/Dhaka");
    ...
    async bulk(input: BulkInput): Promise<BulkResult> {
        ...
        if (!(await hasFullReach("task.edit"))) {
        ...
        if (newGroup !== undefined && DONE_GROUPS.has(newGroup)) {
            for (const t of found) {
                if (t.statusId === p.statusId) continue; // already there
                const blockers = await this.tasks.openBlockerCount(t.id);
                if (blockers > 0) {
                    throw AppError.conflict(
                        "task.cannot_complete_blocked",
                        `Task ${t.id} is blocked by ${blockers} open task(s) — complete or unlink them first`,
                    );
                }
            }
        }
        ...
        let bulkSplit: GateSplit | null = null;
        if (assigneeAdd.length > 0) {
            const spaceInfoForGate = await this.tasks.spaceInfoByTask(ids);
            bulkSplit = await assignmentGate().splitByApproval({
                workspaceId: input.workspaceId,
                requesterId: input.actorId,
                pairs: found.flatMap((t) =>
                    assigneeAdd.map((targetUserId) => ({
                        taskId: t.id,
        ...
        const bulkClearsStartDate =
            p.startDate !== undefined && p.startDate === null;
        if (p.dueTime) {
            const orphan = found.some((t) =>
                timeWithoutDate(resulting(p.dueDate, t.dueDate), p.dueTime),
            );
            if (orphan) refuseTimeWithoutDate("due");
        }
        if (p.startTime) {
            const orphan = found.some((t) =>
                timeWithoutDate(resulting(p.startDate, t.startDate), p.startTime),
            );
            if (orphan) refuseTimeWithoutDate("start");
        }
        ...
        let bulkRequestDeliveries: RequestCreatedDelivery[] = [];
            ...
            await this.db.transaction(async (tx) => {
                ...
                if (assigneeAdd.length > 0 && bulkSplit) {
                    ...
                    for (const g of groups.values()) {
                        ...
                        await this.membership.addWatchersBulk(
                            g.taskIds,
                            g.userIds,
                            tx,
                        );
                    ...
                    for (const t of found) {
                        ...
                        const recipients = gained.filter(
                            (id) => id !== input.actorId,
                        );
                        if (recipients.length > 0) {
                            await this.notifications.createMany(
                    ...
                    const gatedFresh = bulkSplit.gated.filter(
                        (pair) =>
                            !(preAssignees.get(pair.taskId) ?? []).includes(
                                pair.targetUserId,
                            ),
                    );
                    const { created, deliveries } = await assignmentGate()
                ...
                for (const t of found) {
                    ...
                    const statusMoved =
                        p.statusId !== undefined && p.statusId !== t.statusId;
                    if (statusMoved) {
                        bulkRows.push({
                            taskId: t.id,
                            actorId: input.actorId,
