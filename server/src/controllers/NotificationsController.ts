...
import type { PrefUpsert } from "../repositories/NotificationPrefsRepo";
import type {
...
/**
 * §19 Notifications HTTP layer. Controllers translate the validated request into
 * a service call and the service result into the wire shape — collection reads
 * use the spec's `{ data, pagination }` envelope; single-notification reads and
 * mutations return the bare `Notification`; counts/bulk return a small object.
 * Identity (`req.auth.sub`) always comes from the verified token, never input.
 */

/** Validated, sanitized subset of the feed query (see `listNotificationsValidator`). */
interface FeedQuery {
    cursor?: string;
    limit?: number;
}
...
export class NotificationsController {
     ...
     */
    async feed(req: NotificationFeedRequest, res: Response, next: NextFunction) {
        try {
            const { cursor, limit }: FeedQuery = matchedData(req, {
                locations: ["query"],
            });
    ...
    async snooze(
        ...
        try {
            ...
            const { snoozed_until }: { snoozed_until: string } = matchedData(
                req,
                { locations: ["body"] },
            );
    ...
    async updatePreferences(
        ...
        try {
            const body = req.body;
            const prefs: PrefUpsert[] = Object.entries(body).map(
                ([type, pref]) => ({
