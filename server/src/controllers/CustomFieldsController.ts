...
import type { HydratedCustomField } from "../services/CustomFieldsService";
import type { CustomFieldScopeType } from "../repositories/CustomFieldsRepo";
import type { CustomFieldUpdateFields } from "../repositories/CustomFieldsRepo";
import type {
...
/**
 * Wire `CustomField` per API_DESIGN.md Appendix A. snake_case; OMITS
 * workspace_id / hidden_from_guests / created_by / timestamps. `options` is
 * present (inline) for dropdown fields only.
 */
interface WireCustomFieldOption {
    id: string;
    label: string;
    color: string;
    position: number;
}
interface WireCustomField {
    id: string;
...
const toWireCustomField = (h: HydratedCustomField): WireCustomField => {
    const { field, options } = h;
    const wire: WireCustomField = {
        id: field.id,
...
export class CustomFieldsController {
    ...
    async listAll(
        req: ListCustomFieldsRequest,
        res: Response,
        next: NextFunction,
    ) {
        try {
            const { workspaceId } = req.auth;
            const scopeType = req.query.scope_type as
                | CustomFieldScopeType
                | undefined;
            const scopeId =
                typeof req.query.scope_id === "string"
                    ? req.query.scope_id
                    : undefined;
    ...
    async listForList(
        req: ListForListCustomFieldsRequest,
        res: Response,
        next: NextFunction,
    ) {
        try {
    ...
    async create(
        ...
        try {
            ...
            const field = await this.service.create({
                workspaceId,
                actorId,
                scopeType: body.scope_type as CustomFieldScopeType,
    ...
    async update(
        ...
        try {
            ...
            const fields: CustomFieldUpdateFields = {};
    ...
    async remove(req: CustomFieldIdRequest, res: Response, next: NextFunction) {
        try {
            const { sub: actorId, workspaceId } = req.auth;
            const fieldId = req.params.id;
