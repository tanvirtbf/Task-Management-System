    ...
    [
        ...
        },
        {
            ...
            function: {
                name: "search",
                description:
                    "Search the tasks, lists, and spaces THIS USER can see for a keyword (team-scoped members search their own teams' work, not the whole company). It matches item NAMES and text, NOT who a task is assigned to — it cannot list a person's tasks. Use when the user asks to find or locate a specific item by name.",
                parameters: {
                    type: "object",
                    properties: {
                        query: {
                ...
                parameters: {
                    ...
                    properties: {
                        person_name: {
                            type: "string",
                            description:
                                'Name or @handle (e.g. "@arif"); "@me" allowed',
                        },
                        bucket: {
                            type: "string",
                            enum: [
                        ...
                        window_days: {
                            type: "integer",
                            description:
                                "completed only: how many days back (1–92, default 30)",
                        },
                    },
        ...
        },
        {
            ...
            function: {
                name: "get_team_stats",
                description:
                    'One TEAM\'s created / assigned-to / overdue-now / completed counts inside a window, only across tasks the ASKER can see. Use for "X team er last N dine koyta task create hoise", "ke ke assign chilo", "team er overdue koyta". Relay the note; an invisible team answers not_found — never confirm it exists.',
                parameters: {
                    type: "object",
                    properties: {
                        team_name: { type: "string" },
...
> => {
    ...
    const byLocal = rows.filter(
        (u) => u.email.split("@")[0].toLowerCase() === local,
    );
    if (byLocal.length === 1) {
        return {
            person: {
    ...
    const person =
        exactMatch(named, stripped) ?? (named.length === 1 ? named[0] : null);
...
type PersonTaskBucket = (typeof PERSON_TASK_BUCKETS)[number];
 ...
 */
async function personTasksTool(
    args: Record<string, unknown>,
    ctx: ToolContext,
    services: ToolServices,
): Promise<unknown> {
    ...
    let bucket: PersonTaskBucket = "open";
    if (args.bucket !== undefined) {
        if (
            typeof args.bucket === "string" &&
            (PERSON_TASK_BUCKETS as readonly string[]).includes(args.bucket)
        ) {
            bucket = args.bucket as PersonTaskBucket;
        } else {
            return {
...
async function teamStatsTool(
    ...
    if (!team) {
        ...
        const contains = named.filter((s) =>
            s.name.toLowerCase().includes(q),
        );
        if (contains.length === 1) team = contains[0];
    ...
    return {
        ...
        note:
            `Counts cover ONLY tasks YOU can see in "${team.name}" — a scoped view can undercount. ` +
