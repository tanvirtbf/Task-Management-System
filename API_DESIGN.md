...
Known `action` values (the REAL producer vocabulary, verified against every `task_activity` writer — the draft-era list named codes the server never emits): `task_created`, `task_updated` (context `{fields, changes:{field:{from,to}}}` since F21; `bulk: true` on bulk edits; description values clipped to 280 chars since P3), `status_changed` (`{from,to}` status ids + denormalised `from_name`/`to_name` since P3), `assignee_added`/`assignee_removed` (`{user_id}`; `bulk: true` when a bulk edit changed the pair), `tag_added`/`tag_removed` (`{tag_id, name}`), `comment_posted`, `comment_updated`, `comment_deleted` (`{comment_id, author_id}` — an admin deleting someone else's words stays attributable), `comment_referenced`, `attachment_added`/`attachment_removed` (`{attachment_id, name}`), `checklist_created` (`{checklist_id, name}`; `{items, via_template}` when a template spawned it), `checklist_renamed` (`{from,to}`), `checklist_deleted`, `checklist_item_added` (`bulk: true` on the bulk endpoint), `checklist_item_deleted`, `checklist_item_toggled`, `checklist_item_updated` (`{fields, text_from?, text_to?, assignee_from?, assignee_to?}`), `dependency_added`/`dependency_removed`, `sprint_added`/`sprint_removed`/`task_removed`/`sprint_rolled_over`, `task_archived`/`task_unarchived` (`{via_parent}` on cascade rows; `{bulk}` on bulk), `task_reviewed`, `sla_overridden`, `postmortem_submitted` (`{items, revised}`), `created_from_template` (`{template_id, name}`), `custom_field_value_set` (`{field_id, field_name, value}` — value clipped)/`custom_field_value_cleared` (`{field_id, field_name}`).

**Team-access P3 (2026-08-11, upgrades/017): the log is COMPLETE — no change to a task happens without a row.** The operations that used to be invisible now write in the same transaction as their mutation: bulk assignee/tag/(un)archive changes (per pair/task that ACTUALLY changed), attachment add/remove, comment edit/delete, checklist rename + bulk item add + item field detail, postmortem submits, archive-cascade rows on descendants. A **hard delete** writes a `workspace_activity` row (entity_type `'task'`, action `task_hard_deleted`, context `{name, custom_id, list_id, subtree_count}`) BEFORE the cascade — the task's own `task_activity` rows die with it. Deliberately still silent: `POST /uploads/sign` (a pending row nobody finished is not an event), watcher self-(un)subscribe (personal, documented at the route), checklist/list position shuffles (presentation), and INITIAL content at creation (a form-submitted task's custom-field values, a create's initial tags — the creation row covers the birth).
...
### Edit rights (P7, upgrade `020` — LIVE)
**Only assignees, the creator, and the HEAD of the owning space may edit a task** (Member `task.edit`/`task.archive`/`task.delete` = `own`; Admin/Owner `all`; `task.assign` stays open until the P8 approval flow). The head allow-path is object-level: the unified guard resolves task → space → head and permits the head without assignment (verb still required). **One doctrine on every adjacent surface** — mutating a task's CONTENT is editing the task: checklists (all mutations), attachment sign/upload/finalize, custom-field set/clear, dependency link/unlink, SLA override, tags, bulk, archive all enforce `task.edit` reach on the resolved task, whatever the route's own key. Attachment delete stays uploader-or-admin; comments stay open (discussion, author-only edit/delete rules unchanged). Denials are the taxonomy 403 (`task.forbidden` / reason `not_own`) with the human message "You can only edit tasks for items you created or are assigned to."
...
**Delivery (P9, R1.6):** five email + Web Push moments fire post-commit to exactly the in-app bell's recipients — request created → the target + their Heads (mail links the **inbox**: the task is still 404 for them until accept, B5), accepted / declined / query raised → the requester (links the task), query answered → the receiver side (links the inbox). One bilingual template family (`MailService.sendAssignmentRequestEmail`). **G15 fixed:** bulk assignment used to be silent — its direct adds now produce the same `assigned` notification + email/push per (task, person) actually gained. **UI:** the task drawer's "Assignment approval" panel and the Inbox **Requests** filter act on requests inline (accept / decline / query / answer / withdraw); the assignee picker shows team chips and warns "*will need <team>'s approval*" before a cross-team pick; the bulk toolbar reports "N updated — M waiting for approval"; a 404 task deep-link explains the team boundary and points at the Inbox instead of a raw not-found.
