...
/**
 * True when a Drizzle/libsql error is a UNIQUE/PK violation
 * (`SQLITE_CONSTRAINT_UNIQUE` — message `UNIQUE constraint failed: …`). Lets
 * `createForList` translate a `uq_statuses_scope_name` collision into
 * `409 status.duplicate` without a TOCTOU pre-check — the insert is the
 * race-safe arbiter.
 */
...
/**
 * True when a libsql error is a foreign-key violation
 * (`SQLITE_CONSTRAINT_FOREIGNKEY` — message `FOREIGN KEY constraint failed`).
 * Lets `deleteById` translate a `fk_tasks_status` rejection — a task
 * referencing the status raced in after the `countTasksUsingStatus` pre-check
 * — into `409 status.in_use`.
 */
