...
/** libSQL surfaces a unique-violation (`uq_sprints_workspace_name`) as
 *  `SQLITE_CONSTRAINT_UNIQUE` (or a generic `SQLITE_CONSTRAINT` whose message
 *  names the failed UNIQUE constraint). */
...
// Wire dates (`YYYY-MM-DD`) are stored verbatim: SQLite DATE columns are TEXT,
// so no Date-object conversion happens between the API and the row (the old
// MySQL local-midnight `toLocalDate` round-trip is gone).
...
export class SprintsService {
     ...
     *
     * The sprint row is re-read and ALL validation runs inside the transaction
     * (SQLite serialises writing transactions), so the add serialises against
     * `close()`: a concurrent close either snapshots these tasks for rollover, or
