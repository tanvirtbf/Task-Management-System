...
/**
 * Flatten a libsql error (possibly wrapped, possibly chained via `cause`) into
 * one searchable "CODE message" string — libsql reports constraint violations
 * by SQLITE_CONSTRAINT_* code plus a message naming the columns (e.g.
 * `UNIQUE constraint failed: tasks.workspace_id, tasks.custom_id`).
 */
...
/** libsql surfaces a unique-violation as `SQLITE_CONSTRAINT_UNIQUE`/`_PRIMARYKEY`
 *  ("UNIQUE constraint failed: …") — was mysql2's `ER_DUP_ENTRY` / errno 1062. */
...
/** libsql surfaces a missing-FK-target as `SQLITE_CONSTRAINT_FOREIGNKEY`
 *  ("FOREIGN KEY constraint failed") — was `ER_NO_REFERENCED_ROW(_2)` / 1452. */
...
/**
 * Normalise a `YYYY-MM-DD` wire date for a DATE-only column. conv: on SQLite
 * these columns are TEXT ISO `YYYY-MM-DD`, so the (already-canonical, format-
 * validated) wire value passes straight through — no more parsing to a
 * local-midnight `Date` for the mysql2 DATE column.
 */
    ...
    async myWork(
        ...
        for (const { task, statusGroup } of rows) {
            ...
            if (DONE_GROUPS.has(statusGroup)) {
                ...
                // due_date is already a `YYYY-MM-DD` string (TEXT column) —
