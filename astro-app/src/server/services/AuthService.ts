...
/**
 * Transient lock errors worth retrying a transaction on. (conv: was mysql2's
 * errno 1213/1205 deadlock pair; libsql surfaces write contention as
 * `SQLITE_BUSY` / `SQLITE_LOCKED` on the error's `code`.)
 */
...
// A forgot-password DELETE+INSERT pair could deadlock on MySQL under
// concurrent requests for the same user; on SQLite the single writer removes
// the deadlock, but a busy-timeout is still possible — keep the bounded retry
// the same way OnCallService does.
...
export class AuthService {
     ...
     * transaction: all-or-nothing. The transaction serializes concurrent uses
     * of the same token (conv: was a MySQL `FOR UPDATE` row lock; SQLite's
     * single writer covers it), so the "unconsumed?" check is race-free and a
     * concurrent replay of the same token writes nothing and fails.
     */
    ...
    async forgotPassword(input: { email: string }): Promise<void> {
        ...
        //
        // (conv: on MySQL the DELETE (a `user_id` range) + INSERT pair could
        // deadlock when several forgot-password requests for the SAME user
        // raced — gap locks, ER_LOCK_DEADLOCK 1213. SQLite's single writer
        // removes the deadlock; the bounded retry is kept as a backstop for
        // transient SQLITE_BUSY contention, turning it into the same 202 the
        // caller expects (mirrors OnCallService's tx-retry).) `rawToken` is
        // fixed across attempts so the emailed link stays valid; each attempt
        // mints a fresh row id.
        let lastErr: unknown;
        for (let attempt = 0; attempt < MAX_FORGOT_TX_ATTEMPTS; attempt++) {
            try {
                await this.db.transaction(async (tx) => {
                    // Serialize concurrent forgot-password for the SAME user.
                    // (conv: on MySQL this read took the user-row lock FIRST
                    // (lock-ordering) so parallel DELETE+INSERT pairs couldn't
                    // deadlock on the password_reset_tokens `user_id` gap locks
                    // (ER_LOCK_DEADLOCK 1213). SQLite has no FOR UPDATE and a
                    // single writer, so the transaction itself serializes; the
                    // read is kept to preserve the flow's structure and the
                    // retry below stays as a defensive backstop.)
     ...
     * reads the invitation (serialized — conv: was a `FOR UPDATE` lock, now
     * SQLite's single writer), sets the invited user's first password (bcrypt),
    ...
    async acceptInvitation(input: {
        ...
        const accepted = await this.db.transaction(async (tx) => {
            ...
            // workspace + email). Read it inside the transaction so a
            // concurrent deactivate / role change serializes with the accept.
