...
export interface DayCount {
    /** `YYYY-MM-DD` (UTC, via date() over the epoch-ms column). */
    day: string;
    cnt: number;
}

/** The grouped-by-creation-day select shared by every KPI series query. */
// conv: was DATE_FORMAT(created_at, '%Y-%m-%d') — created_at is epoch ms now.
const DAY = sql<string>`date(${tasks.createdAt} / 1000, 'unixepoch')`;
