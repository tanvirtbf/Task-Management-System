...
export class PasswordResetTokensRepo {
    ...
    /**
     * Look up a token row by its `sha256` hash for the serialized consume
     * flow: concurrent uses of the SAME token cannot race the service's
     * "unconsumed?" check and the consume write — a replay sees `consumed_at`
     * already set. Must run inside a transaction; pass the `tx` executor.
     * (conv: was a MySQL `FOR UPDATE` row lock; SQLite has no FOR UPDATE — the
     * write transaction's single-writer serialization gives the same guarantee.)
     */
