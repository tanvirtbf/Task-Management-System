...
export class TaskMembershipRepo {
    ...
    /**
     * Insert assignee rows. The `ON CONFLICT DO NOTHING` (was a MySQL
     * `ON DUPLICATE KEY UPDATE` no-op) makes a concurrent re-insert of the same
     * `(task_id, user_id)` a harmless no-op instead of a duplicate-key error.
     */
    async addAssignees(
        taskId: string,
        userIds: string[],
     ...
     * was inserted, `false` on a re-watch no-op — `rowsAffected` is 1 for an
     * insert and 0 when the `ON CONFLICT DO NOTHING` skips an existing row.
     */
    async addWatcher(
        taskId: string,
        userId: string,
