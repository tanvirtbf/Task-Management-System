...
/**
 * SQLite hands `size_bytes` back as a JS number (INTEGER column); the
 * app-level record keeps carrying the bigint it always had, so callers
 * (service, serializer) stay unchanged.
 */
type AttachmentSelectRow = Omit<AttachmentRecord, "sizeBytes"> & {
    sizeBytes: number;
};

const toRecord = (row: AttachmentSelectRow): AttachmentRecord => ({
    ...row,
...
export class AttachmentsRepo {
    ...
    async markComplete(
        ...
        if (opts.sizeBytes != null) patch.sizeBytes = Number(opts.sizeBytes);
    ...
    async softDelete(
        ...
        const result = await exec
            .update(attachments)
            .set({
                deletedAt: sql`(cast(unixepoch('subsec') * 1000 as integer))`,
            })
