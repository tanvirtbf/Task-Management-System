...
/**
 * Escape LIKE wildcards so a user's `q` matches literally. Without this, a
 * search for "50%" or "a_b" would behave as a wildcard pattern. SQLite has no
 * default escape character (MySQL's was backslash), so the pattern must be
 * paired with an explicit `ESCAPE '\'` — see `likeEscaped`.
 */
...
export class UsersRepo {
    ...
    /**
     * Case-insensitive lookup. The `users.email` column is declared
     * `TEXT COLLATE NOCASE` (preserving MySQL's `utf8mb4_unicode_ci` semantics
     * for ASCII), so the equality compares case-insensitively and the
     * `idx_users_email` index is used.
     *
    ...
    /**
     * Workspace-scoped single-user read for serialized read-merge-write — same
     ...
     * atomically: a second identical concurrent writer blocks, then re-reads
     * the post-change row and short-circuits — one logical transition, one audit
     * row. (conv: was a MySQL `FOR UPDATE` row lock; SQLite has no FOR UPDATE —
     * the write transaction's single-writer serialization gives the same
     * guarantee, so the lock clause is gone but the name and contract stay.)
     */
    ...
    /**
     * Workspace+email lookup used by the invitation-accept flow to atomically
     * flip the invited user to `active`. Must run inside a transaction (pass
     * `exec`) — the transaction serializes concurrent accepts (conv: was a
     * MySQL `FOR UPDATE` row lock; SQLite's single writer covers it).
     * `(workspace_id, email)` is UNIQUE so this resolves at most one row;
     * returns the minimal shape the accept flow needs to validate status and
     * mint the session.
     */
     ...
     * a UNIQUE-constraint error (`SQLITE_CONSTRAINT_UNIQUE`; was mysql2's
