...
/**
 * SQLite hands `internal_id` back as a JS number (INTEGER PK); the app-level
 * row keeps carrying the bigint it always had, so callers (§19 service, §27
 * SSE controller) stay unchanged.
 */
type NotificationSelectRow = Omit<NotificationRow, "internalId"> & {
    internalId: number;
};

export const toNotificationRow = (
    row: NotificationSelectRow,
): NotificationRow => ({
    ...row,
    internalId: BigInt(row.internalId),
});
...
export class NotificationsRepo {
     ...
     * goes through the same driver epoch-ms mapping as the stored
