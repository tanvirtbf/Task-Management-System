...
export class EngineeringRepo {
     ...
     * "This week" = today (`date('now')`, UTC — the intentional fix for the old
     * CURDATE() session-timezone bug) within `[week_start, week_end]`; the most
     * recent matching shift wins. Filtering to `status = 'active'` avoids handing
     * an assignment to a deactivated user (which `create()` would reject as 422).
     */
        ...
        const rows = await this.db
            ...
            .where(
                and(
                    ...
                    sql`date('now') BETWEEN ${onCallShifts.weekStart} AND ${onCallShifts.weekEnd}`,
                ),
            )
     ...
     * default. Evaluated DB-side (`unixepoch()` vs the epoch-ms column) to avoid
     * app/DB clock skew.
     */
        ...
        const rows = await this.db
            ...
            .where(
                and(
                    ...
                    isNull(tasks.archivedAt),
                    notInArray(statuses.statusGroup, [...DONE_GROUPS]),
                    sql`${tasks.updatedAt} < (cast(unixepoch('subsec') * 1000 as integer) - ${sql.raw(String(STALE_DAYS))} * 86400000)`,
                ),
            )
     ...
     * (the `updated_at` bump is explicit — the conflict-update path bypasses the
     * schema's `$onUpdate`).
     */
