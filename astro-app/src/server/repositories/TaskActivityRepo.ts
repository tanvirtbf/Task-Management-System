 ...
 * monotonic keyset / cursor value (INTEGER PK autoincrement); `actorId` is
 * `null` for system events or when the actor's user row was deleted
 * (`fk_task_activity_actor` is `ON DELETE SET NULL`).
 */
    ...
    private feedWhere(params: ActivityFeedParams): SQL | undefined {
        return and(
            ...
            params.afterInternalId
                ? lt(taskActivity.internalId, Number(params.afterInternalId))
                : undefined,
        );
    }
}
