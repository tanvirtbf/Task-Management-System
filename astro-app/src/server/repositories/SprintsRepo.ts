...
export interface InsertSprintInput {
    ...
    name: string;
    goal: string | null;
    /** ISO `YYYY-MM-DD` (SQLite DATE columns are TEXT). */
    startDate: string;
    endDate: string;
...
export class SprintsRepo {
    ...
    /** Resolve a sprint row for update (in the caller's workspace) — null if
     *  absent. SQLite has no `FOR UPDATE`; writes are serialised by the engine,
     *  so this is a plain transactional read. */
    ...
    /**
     * Serialise sprint state transitions (start / close) per workspace, making
     * the "exactly one active sprint" invariant race-free without a dedicated
     * unique constraint. Was a `SELECT … FOR UPDATE` row lock on MySQL; SQLite
     * has a single writer and serialises transactions, so a plain read inside
     * the caller's transaction suffices.
     */
