...
export interface OnCallShiftRecord {
    id: string;
    /** SQLite DATE column — TEXT `YYYY-MM-DD` (serializer passes it through). */
...
/**
 * Normalise a week bound to the ISO `YYYY-MM-DD` TEXT the SQLite DATE column
 * stores. A Date (the service passes local midnight) is formatted from LOCAL
 * components so it round-trips without a UTC-boundary shift; a pre-formatted
...
export class OnCallRepo {
    ...
    /**
     * The shift covering today in a workspace — `week_start <= date('now') <=
     ...
     * engineer join in one round-trip. "Today" is `date('now')` (UTC) compared
     * via raw `sql` against the ISO-text DATE columns (no JS date math; UTC is
     * intentional — it fixes the old CURDATE() session-timezone bug).
     */
        ...
        const [row] = await this.db
            ...
            .where(
                and(
                    ...
                    sql`${onCallShifts.weekStart} <= date('now')`,
                    sql`${onCallShifts.weekEnd} >= date('now')`,
                ),
            )
     ...
     * `week_start` (passed as `YYYY-MM-DD`; ISO text compares lexicographically =
     * chronologically — no JS date math). Filtering is on `week_start`,
     ...
     * change (+ an explicit `updated_at` bump — the upsert path bypasses the
     * schema's `$onUpdate`), so the original creator + creation time are
     * preserved. `week_start` / `week_end` are accepted as local-midnight Dates
     * or `YYYY-MM-DD` strings and stored as ISO text (SQLite DATE columns).
     * Pass `exec` to enlist in the caller's transaction so the paired re-read
     * sees this write atomically.
     */
    async upsert(
        input: {
            workspaceId: string;
        ...
        const id = fakeId("ocs");
        const weekStart = toIsoDate(input.weekStart);
        const weekEnd = toIsoDate(input.weekEnd);
        await exec
            .insert(onCallShifts)
            .values({
                id,
            ...
            .onConflictDoUpdate({
                target: [onCallShifts.workspaceId, onCallShifts.weekStart],
                set: {
                    engineerId: input.engineerId,
                    weekEnd,
                    updatedAt: new Date(),
     ...
     * raw `sql` against the ISO-text DATE column. Pass `exec` to read inside the
     ...
     * against the ISO-text DATE column. The workspace-scoped `WHERE` is the
